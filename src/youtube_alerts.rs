        ...
        if let Some(refresh) = tokens.refresh_token.clone() {
            if let Some(app) = fetch_or_seed_youtube_oauth_app_config(pool, tenant_id).await? {
                    ...
                    .filter(|v| !v.is_empty())
                {
                    ...
                    update_youtube_connection_tokens(pool, tenant_id, channel_id, &refreshed)
                        .await?;
...
pub async fn evaluate_youtube_alerts(
    ...
    let total_rev_7d = if cur_rev.is_finite() {
        Some(cur_rev)
    } else {
        None
    };
    ...
    if top_video_7d.is_none() && total_rev_7d.unwrap_or(0.0) >= 20.0 {
        ...
        if let Some(access_token) =
            best_effort_youtube_access_token(pool, tenant_id, channel_id).await?
        {
            if let Ok(rows) = fetch_top_videos_by_revenue_for_channel(
                &access_token,
                channel_id,
                current_start,
                current_end,
