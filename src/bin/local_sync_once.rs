...
async fn main() -> Result<(), Error> {
    ...
    let mut tokens =
        match fetch_youtube_connection_tokens(pool, tenant_id.trim(), channel_id.trim()).await? {
            Some(v) => v,
            None => {
                eprintln!(
                    "Missing YouTube connection tokens for tenant/channel. Reconnect OAuth first."
                );
                return Ok(());
            }
        };
    ...
    if needs_refresh {
        if let Some(refresh) = tokens.refresh_token.clone() {
            ...
            update_youtube_connection_tokens(pool, tenant_id.trim(), channel_id.trim(), &refreshed)
                .await?;
    ...
    let thumb_video_only_res =
        fetch_report_json_by_url(&tokens.access_token, &url_thumb_video_only).await;
        ...
        Err(err) if err.status == Some(401) => {
            if let Some(refresh) = tokens.refresh_token.clone() {
                ...
                let client_secret = app
                    ...
                    .filter(|v| !v.is_empty())
                    .ok_or_else(|| {
                        Box::new(std::io::Error::other("missing youtube oauth client_secret"))
                            as Error
                    })?;
    ...
    let (max_dt, sum_views, sum_impressions, sum_revenue): (Option<NaiveDate>, i64, i64, f64) =
        sqlx::query_as(
            r#"
