...
pub struct RiskCheckPrompt {
    pub system: String,
    pub user: String,
}

pub fn build_risk_check_prompt(args: RiskCheckMessageArgs<'_>) -> RiskCheckPrompt {
...
pub fn build_risk_check_messages(
    args: RiskCheckMessageArgs<'_>,
) -> Result<Vec<ChatCompletionRequestMessage>, Error> {
    let prompt = build_risk_check_prompt(args);
    ...
    let user_msg = ChatCompletionRequestUserMessageArgs::default()
        .content(ChatCompletionRequestUserMessageContent::Text(prompt.user))
        .build()
        .map_err(|e| -> Error { Box::new(std::io::Error::other(e.to_string())) })?;
