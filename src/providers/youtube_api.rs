...
pub async fn fetch_my_channel_id_with_base_url(
    access_token: &str,
    base_url: &str,
) -> Result<String, Error> {
    ...
    let channel_id = json
        ...
        .filter(|v| !v.is_empty())
        .ok_or_else(|| {
            Box::new(std::io::Error::other("No channel_id found for this token")) as Error
        })?;

    Ok(channel_id)
}

pub async fn list_my_channels_with_base_url(
    access_token: &str,
    base_url: &str,
) -> Result<Vec<MyChannelSummary>, Error> {
    let base = base_url.trim_end_matches('/');
...
mod tests {
    ...
    async fn fetches_channel_id_via_sdk_against_mock_server() {
        ...
        let channel_id = fetch_my_channel_id_with_base_url("token123", &base_url)
            .await
            .unwrap();
    ...
    async fn lists_channels_via_sdk_against_mock_server() {
        ...
        let channels = list_my_channels_with_base_url("token123", &base_url)
            .await
            .unwrap();
