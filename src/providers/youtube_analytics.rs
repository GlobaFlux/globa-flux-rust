...
impl std::fmt::Display for YoutubeAnalyticsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(status) = self.status {
            write!(
                f,
                "YouTube Analytics error (status {status}): {}",
                self.message
            )
        } else {
...
fn build_channel_reports_url_views_only(
    base_url: &str,
    start_dt: NaiveDate,
    end_dt: NaiveDate,
) -> String {
...
fn parse_rows(json: &Value) -> Vec<VideoDailyMetricRow> {
    ...
    for row in rows {
        ...
        let impressions_ctr = idx_ctr.and_then(|i| arr.get(i)).and_then(|v| {
            v.as_f64()
                .or_else(|| v.as_str().and_then(|s| s.parse().ok()))
        });
...
async fn fetch_video_totals_for_ids_with_base_url(
    ...
    let url = build_video_totals_url_with_ids_and_metrics(
        base_url, ids_value, start_dt, end_dt, metrics, sort, limit,
    );
...
async fn fetch_top_videos_by_revenue_for_ids_with_base_url(
    ...
    .await
    {
        ...
        Err(err) if should_fallback_to_views_only(&err) => {
            fetch_video_totals_for_ids_with_base_url(
                access_token,
                base_url,
                ids_value,
                start_dt,
...
async fn fetch_video_daily_metrics_for_ids_with_base_url(
    ...
    fn compute_channel_totals_from_video_rows(
    ...
    match fetch_report_json_by_url(access_token, &video_url).await {
        Ok(json) => {
            ...
            if !parsed.is_empty() {
                ...
                let video_url =
                    build_reports_url_with_ids_views_only(base_url, ids_value, start_dt, end_dt);
                match fetch_report_json_by_url(access_token, &video_url).await {
                    Ok(json) => {
                        let parsed = parse_rows(&json);
                        if !parsed.is_empty() {
    ...
    if !video_rows.is_empty() {
        ...
        let impressions_url =
            build_reports_url_with_ids_impressions(base_url, ids_value, start_dt, end_dt);
        ...
        if !has_channel_totals {
            let channel_url =
                build_channel_reports_url_with_ids(base_url, ids_value, start_dt, end_dt);
            let mut totals_rows = match fetch_report_json_by_url(access_token, &channel_url).await {
                Ok(json) => parse_rows_channel(&json),
                Err(err) if should_fallback_to_views_only(&err) => {
                    let channel_url = build_channel_reports_url_with_ids_views_only(
            ...
            let channel_impressions_url = build_channel_reports_url_with_ids_impressions(
                base_url, ids_value, start_dt, end_dt,
            );
...
async fn fetch_video_daily_metrics_for_channel_with_base_url(
    ...
    fetch_video_daily_metrics_for_ids_with_base_url(
        access_token,
        base_url,
        &ids_value,
        start_dt,
...
mod tests {
    ...
    fn build_reports_url_for_thumbnail_impressions_includes_expected_metrics() {
        ...
        assert!(
            url.contains("metrics=videoThumbnailImpressions,videoThumbnailImpressionsClickRate")
        );
    }
    ...
    fn build_channel_reports_url_includes_expected_params() {
        ...
        let end_dt = NaiveDate::from_ymd_opt(2026, 1, 7).unwrap();
        let url =
            build_channel_reports_url("https://youtubeanalytics.googleapis.com/", start_dt, end_dt);
    ...
    async fn falls_back_to_views_only_video_report_when_query_not_supported() {
        ...
        let start_dt = NaiveDate::from_ymd_opt(2026, 1, 1).unwrap();
        let end_dt = NaiveDate::from_ymd_opt(2026, 1, 7).unwrap();
        let rows = fetch_video_daily_metrics_for_channel_with_base_url(
            "token123", &base_url, "UC123", start_dt, end_dt,
        )
        .await
        .unwrap();
    ...
    async fn serve_reports_with_dimensionless_reach_fallback(
