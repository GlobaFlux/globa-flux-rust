...
impl std::fmt::Display for YoutubeReportingError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(status) = self.status {
            write!(
                f,
                "YouTube Reporting error (status {status}): {}",
                self.message
            )
        } else {
...
pub fn build_reports_list_url(
    ...
    let base = base_url.trim_end_matches('/');
    let mut url = format!("{base}/jobs/{job_id}/reports?onBehalfOfContentOwner={content_owner_id}");
...
pub async fn ensure_job_for_report_type_with_base_url(
    ...
    create_job_for_report_type_with_base_url(
        access_token,
        content_owner_id,
        report_type_id,
        base_url,
...
pub async fn ensure_job_for_report_type(
    ...
    ensure_job_for_report_type_with_base_url(
        access_token,
        content_owner_id,
        report_type_id,
        DEFAULT_BASE_URL,
...
pub async fn ensure_job_for_report_type_channel(
    ...
    ensure_job_for_report_type_channel_with_base_url(access_token, report_type_id, DEFAULT_BASE_URL)
        .await
}
...
pub async fn list_report_types_channel(
    ...
    list_report_types_channel_with_base_url(access_token, DEFAULT_BASE_URL, include_system_managed)
        .await
}
...
mod tests {
    ...
    fn builds_report_types_list_url_with_content_owner() {
        let url =
            build_report_types_list_url("https://youtubereporting.googleapis.com/v1/", "CMS123");
        assert_eq!(
            url,
        ...
        http1::Builder::new()
            .serve_connection(
                ...
                service_fn(|req: Request<Incoming>| async move {
                    ...
                    let body =
                        r#"{"reportTypes":[{"id":"rt1","name":"Report 1","systemManaged":true}]}"#;
                    Ok::<_, hyper::Error>(
                        Response::builder()
    ...
    fn parses_reports_from_list_response() {
        ...
        assert_eq!(reports[0].end_time.as_deref(), Some("2026-01-02T00:00:00Z"));
        assert_eq!(
            ...
            .serve_connection(
                ...
                service_fn(|req: Request<Incoming>| async move {
                    ...
                    let path = req
                        ...
                        .map(|pq| pq.as_str())
                        .unwrap_or("");
                    if req.method() != Method::POST || path != "/jobs?onBehalfOfContentOwner=CMS123"
                    {
                        return Ok::<_, hyper::Error>(
                            Response::builder()
    ...
    async fn creates_job_for_report_type_against_mock_server() {
        ...
        let job_id =
            create_job_for_report_type_with_base_url("token123", "CMS123", "rt1", &base_url)
                .await
                .unwrap();
