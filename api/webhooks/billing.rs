...
fn extract_provider_subscription_id(payload: &Value) -> Option<String> {
    if let Some(gid) = payload.get("admin_graphql_api_id").and_then(|v| v.as_str()) {
        return Some(gid.to_string());
    }
    payload
        .get("id")
...
async fn handle_billing(
    ...
    if let Ok(payload) = serde_json::from_str::<Value>(&parsed.raw_body) {
        ...
        let status = normalize_subscription_status(
            &parsed.topic,
...
async fn handle_subscription_status(
    ...
    json_response(
        StatusCode::OK,
        serde_json::json!({"ok": true, "subscription": payload}),
    )
}
