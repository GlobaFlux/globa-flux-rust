...
fn llm_settings_gone_response() -> Result<Response<ResponseBody>, Error> {
    json_response(
        StatusCode::GONE,
        serde_json::json!({
          "ok": false,
          "error": "deprecated",
          "message": "Legacy llm_settings endpoint is removed. Use /api/tenants/ai_settings for tenant BYOK configuration."
        }),
    )
}
...
async fn handle_llm_settings(
    ...
    match *method {
        Method::GET | Method::PUT | Method::POST => llm_settings_gone_response(),
        _ => json_response(
            StatusCode::METHOD_NOT_ALLOWED,
            serde_json::json!({"ok": false, "error": "method_not_allowed"}),
        ),
...
mod tests {
    ...
    #[tokio::test]
    async fn returns_gone_for_legacy_llm_settings_get() {
        std::env::set_var("RUST_INTERNAL_TOKEN", "secret");
        ...
        let uri: hyper::Uri = "/api/tenants/llm_settings?tenant_id=global".parse().unwrap();
        let response = handle_llm_settings(&Method::GET, &headers, &uri, Bytes::new())
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn returns_gone_for_legacy_llm_settings_put() {
        std::env::set_var("RUST_INTERNAL_TOKEN", "secret");
        std::env::remove_var("TIDB_DATABASE_URL");
        std::env::remove_var("DATABASE_URL");
        ...
        let mut headers = HeaderMap::new();
        headers.insert("authorization", "Bearer secret".parse().unwrap());
        let uri: hyper::Uri = "/api/tenants/llm_settings".parse().unwrap();
        ...
        let response = handle_llm_settings(&Method::PUT, &headers, &uri, Bytes::from(body))
            .await
            .unwrap();
        assert_eq!(response.status(), StatusCode::GONE);
    }
