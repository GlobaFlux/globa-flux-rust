...
use globa_flux_rust::providers::openai::{
  build_chat_completions_request, build_risk_check_messages, openai_client, openai_client_with_idempotency, pricing_for_model,
...
async fn handler(req: Request) -> Result<Response<ResponseBody>, Error> {
  ...
  if let Some(existing) = fetch_usage_event(pool, &parsed.tenant_id, EVENT_TYPE, &idempotency_key).await? {
    if stream {
      return sse_response(
        StatusCode::OK,
        sse_event(
  ...
  let max_completion_tokens: u32 = std::env::var("OPENAI_MAX_COMPLETION_TOKENS")
    .ok()
  ...
  let pricing: ModelPricingUsdPerMToken = pricing_for_model(&model).ok_or_else(|| {
  ...
  let reserved_cost_usd = compute_cost_usd(pricing, prompt_reserve_tokens, max_completion_tokens);
  ...
  if stream {
    ...
    tokio::spawn(async move {
        ...
        Err(e) => {
          let _ = tx
            .send(Ok(Frame::data(Bytes::from(sse_event(
              "error",
              &serde_json::json!({"code":"config_error","message":e.to_string()}).to_string(),
            )))))
            .await;
          return;
        }
      ...
      let cost_usd =
        compute_cost_usd(pricing, usage.prompt_tokens as u32, usage.completion_tokens as u32);

      let result_json = serde_json::from_str::<serde_json::Value>(output_text.trim()).unwrap_or_else(|_| {
        serde_json::json!({
          "risk_level": "medium",
          "allowed": true,
      ...
      if let Err(err) = insert_result {
        if err.as_database_error().is_some_and(|e| e.is_unique_violation()) {
          ...
          let _ = tx
            .send(Ok(Frame::data(Bytes::from(sse_event(
              "error",
              &serde_json::json!({"code":"tidb_error","message":err.to_string()}).to_string(),
            )))))
            .await;
          return;
        }
  ...
  // Non-streaming mode (optional): call OpenAI once and return JSON.
  let api_key = std::env::var("OPENAI_API_KEY")
    .map_err(|_| Box::new(std::io::Error::other("Missing OPENAI_API_KEY")) as Error)?;
  ...
  let usage = Usage {
    prompt_tokens: response.usage.as_ref().map(|u| u.prompt_tokens).unwrap_or(0) as i32,
    completion_tokens: response.usage.as_ref().map(|u| u.completion_tokens).unwrap_or(0) as i32,
  };
  let cost_usd = compute_cost_usd(pricing, usage.prompt_tokens as u32, usage.completion_tokens as u32);

  let result_json = serde_json::from_str::<serde_json::Value>(&content).unwrap_or_else(|_| {
    serde_json::json!({"risk_level":"medium","allowed":true,"reason":content})
  });
